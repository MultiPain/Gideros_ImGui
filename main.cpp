...
void LUA_THROW_ERROR(lua_State* L, const char* MSG)
{
    lua_pushstring(L, MSG);
    lua_error(L);
}

void LUA_ASSERT(lua_State* L, bool EXP, const char* MSG)
{
    if (!(EXP))
    {
        lua_pushstring(L, MSG);
...
void LUA_FASSERT(lua_State* L, bool EXP, const char* FMT, ...)
{
    if (!(EXP))
    {
...
float getLogicalScaleX(lua_State *L)
{
    lua_getglobal(L, "application");
...
float getLogicalScaleY(lua_State *L)
{
    lua_getglobal(L, "application");
...
{
    lua_Number sx = getLogicalScaleX(L);
    lua_Number sy = getLogicalScaleY(L);
    return ImVec2(sx, sy);
}
...
ImVec2 getMousePos(SpriteProxy* proxy, float x, float y, ImVec2 r_app_scale, ImVec2 app_bounds)
{
...
{
public:
    GidImGui(LuaApplication* application, lua_State* L, bool add_listeners);
    ~GidImGui();
...
public:
    ...
    void applicationResize(Event *)
    {
        ...
        lua_printf(L, "resize [%.2f, %.2f]", app_scale.x, app_scale.y);
...
GidImGui::GidImGui(LuaApplication* application, lua_State* L, bool add_listeners = true)
{
...
int initImGui(lua_State* L)
{
    ...
    Binder binder(L);
    GidImGui* imgui = new GidImGui(application, L, luaL_optboolean(L, 1, 1));
...
int LogToFile(lua_State* L)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    LUA_ASSERT(L, io.LogFilename != NULL, "Log to file is disabled! Use ImGui:setLogFilename(filename) first.");
...
FontData getFontData(lua_State* L, const char* filename)
{
    ...
    if (L)
        LUA_FASSERT(L, data != nullptr, "Cant load '%s' font! File not found.", filename);
