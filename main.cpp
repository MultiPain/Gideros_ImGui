...
static ImVec2 getMousePos(SpriteProxy* proxy, float x, float y, ImVec2 app_scale, ImVec2 r_app_scale, ImVec4 app_bounds)
{
    float nx = 0.0f;
    float ny = 0.0f;
    ...
    //ImVec2((nx + app_bounds.x) * 1.0f / proxy->scaleX(), (ny + app_bounds.y) * 1.0f / proxy->scaleY());
    return getTranslatedMousePos(proxy, nx + app_bounds.x, ny + app_bounds.y);
}
...
public:
    ...
    void onMouseUpOrDown(float x, float y, int button, bool state)
    {
        ...
        io.MousePos = getMousePos(proxy, x, y, app_scale, r_app_scale, app_bounds);
...
int ImGui_impl_setAutoscale(lua_State* L)
{
    GidImGui* imgui = getImgui(L);
    imgui->autoscale = lua_toboolean(L, 2);
    if (imgui->autoscale)
...
int ImGui_impl_applicationResize(lua_State* L)
{
    GidImGui* imgui = getImgui(L);
    imgui->eventListener->applicationResize(nullptr);
    return 0;
...
int loader(lua_State* L)
{
    ...
    const luaL_Reg imguiFunctionList[] =
    {
        ...
        {"onAppResize", ImGui_impl_applicationResize},
