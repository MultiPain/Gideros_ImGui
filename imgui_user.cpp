...
namespace ImGui
{
    ...
    void Internal_DockBuilderDockWindow(const char* window_name, ImGuiID node_id)
    {
        ImGui::DockBuilderDockWindow(window_name, node_id);
    }

    ImGuiDockNode* Internal_DockBuilderGetNode(ImGuiID node_id)
    {
        return ImGui::DockBuilderGetNode(node_id);
    }

    void Internal_DockBuilderSetNodePos(ImGuiID node_id, ImVec2 pos)
    {
        ImGui::DockBuilderSetNodePos(node_id, pos);
    }

    void Internal_DockBuilderSetNodeSize(ImGuiID node_id, ImVec2 size)
    {
        ImGui::DockBuilderSetNodeSize(node_id, size);
    }

    ImGuiID Internal_DockBuilderAddNode(ImGuiID node_id = 0, ImGuiDockNodeFlags flags = 0)
    {
        return ImGui::DockBuilderAddNode(node_id, flags);
    }

    void Internal_DockBuilderRemoveNode(ImGuiID node_id)
    {
        ImGui::DockBuilderRemoveNode(node_id);
    }

    void Internal_DockBuilderRemoveNodeChildNodes(ImGuiID node_id)
    {
        ImGui::DockBuilderRemoveNodeChildNodes(node_id);
    }

    void Internal_DockBuilderRemoveNodeDockedWindows(ImGuiID node_id, bool clear_settings_refs = true)
    {
        ImGui::DockBuilderRemoveNodeDockedWindows(node_id, clear_settings_refs);
    }

    ImGuiID Internal_DockBuilderSplitNode(ImGuiID id, ImGuiDir split_dir, float size_ratio_for_node_at_dir, ImGuiID* out_id_at_dir, ImGuiID* out_id_at_opposite_dir)
    {
        return ImGui::DockBuilderSplitNode(id, split_dir, size_ratio_for_node_at_dir, out_id_at_dir, out_id_at_opposite_dir);
    }

    void Internal_DockBuilderCopyNode(ImGuiID src_node_id, ImGuiID dst_node_id, ImVector<ImGuiID>* out_node_remap_pairs)
    {
        ImGui::DockBuilderCopyNode(src_node_id, dst_node_id, out_node_remap_pairs);
    }

    void Internal_DockBuilderCopyWindowSettings(const char* src_name, const char* dst_name)
    {
        ImGui::DockBuilderCopyWindowSettings(src_name, dst_name);
    }

    void Internal_DockBuilderCopyDockSpace(ImGuiID src_dockspace_id, ImGuiID dst_dockspace_id, ImVector<const char*>* in_window_remap_pairs)
    {
        ImGui::DockBuilderCopyDockSpace(src_dockspace_id, dst_dockspace_id, in_window_remap_pairs);
    }

    void Internal_DockBuilderFinish(ImGuiID node_id)
    {
