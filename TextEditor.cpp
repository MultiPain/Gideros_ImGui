...
#include "imgui_src/imgui.h" // for imGui::GetCurrentWindow()
...
void TextEditor::SetLanguageDefinition(const LanguageDefinition & aLanguageDef)
{
    ...
    for (std::pair<std::string, TextEditor::PaletteIndex>& r : mLanguageDefinition.mTokenRegexStrings)
...
void TextEditor::SetPaletteColor(unsigned int index, ImU32 color)
{
...
{
    int line = aValue.mLine;
    int column = aValue.mColumn;
...
void TextEditor::Advance(Coordinates & aCoordinates) const
{
    ...
    {
        const Line& line = mLines[aCoordinates.mLine];
...
void TextEditor::DeleteRange(const Coordinates & aStart, const Coordinates & aEnd)
{
    ...
    {
        Line& line = mLines[aStart.mLine];
        int n = GetLineMaxColumn(aStart.mLine);
        if (aEnd.mColumn >= n)
    ...
    {
        Line& firstLine = mLines[aStart.mLine];
        Line& lastLine = mLines[aEnd.mLine];
...
int TextEditor::InsertTextAt(Coordinates& /* inout */ aWhere, const char * aValue)
{
    ...
    while (*aValue != '\0')
    {
        ...
        else if (*aValue == '\n')
        {
            ...
            {
                Line& newLine = InsertLine(aWhere.mLine + 1);
                Line& line = mLines[aWhere.mLine];
...
TextEditor::Coordinates TextEditor::ScreenPosToCoordinates(const ImVec2& aPosition) const
{
    ...
    {
        const Line& line = mLines.at(lineNo);
        ...
        while ((size_t)columnIndex < line.size())
        {
            ...
            {
                char buf[7];
                int d = UTF8CharLength(line[columnIndex].mChar);
                int i = 0;
                while (i < 6 && d-- > 0)
...
TextEditor::Coordinates TextEditor::FindWordStart(const Coordinates & aFrom) const
{
    ...
    const Line& line = mLines[at.mLine];
    ...
    while (cindex > 0)
    {
        unsigned char c = line[cindex].mChar;
...
TextEditor::Coordinates TextEditor::FindNextWord(const Coordinates & aFrom) const
{
    ...
    while (!isword || skip)
    {
        if (at.mLine >= (int)mLines.size())
        {
            int l = std::max(0, (int) mLines.size() - 1);
...
int TextEditor::GetCharacterColumn(int aLine, int aIndex) const
{
    if (aLine >= (int)mLines.size())
        return 0;
    const Line& line = mLines[aLine];
    int col = 0;
    int i = 0;
    while (i < aIndex && i < (int)line.size())
    {
        unsigned char c = line[i].mChar;
        i += UTF8CharLength(c);
        if (c == '\t')
...
void TextEditor::RemoveLine(int aStart, int aEnd)
{
    ...
    ErrorMarkers etmp;
    for (std::pair<const int, std::string>& i : mErrorMarkers)
    {
...
TextEditor::Line& TextEditor::InsertLine(int aIndex)
{
    ...
    std::vector<Glyph, std::allocator<Glyph>>& result = *mLines.insert(mLines.begin() + aIndex, Line());
...
ImU32 TextEditor::GetGlyphColor(const Glyph & aGlyph) const
{
    ...
    unsigned int const color = mPalette[(int)aGlyph.mColorIndex];
    if (aGlyph.mPreprocessor)
    {
        const unsigned int ppcolor = mPalette[(int)PaletteIndex::Preprocessor];
...
void TextEditor::Render()
{
    ...
    if (!mLines.empty())
    {
        ...
        while (lineNo <= lineMax)
        {
            ...
            // Draw error markers
            ErrorMarkers::iterator errorIt = mErrorMarkers.find(lineNo + 1);
            ...
            if (mState.mCursorPosition.mLine == lineNo)
            {
                ...
                if (focused)
                {
                    ...
                    unsigned long long elapsed = timeEnd - mStartTime;
                    if (elapsed > 400)
                    {
                        float width = 1.0f;
                        ...
                        if (mOverwrite && cindex < (int)line.size())
                        {
                            ...
                            if (c == '\t')
                            {
                                float x = (1.0f + std::floor((1.0f + cx) / (float(mTabSize) * spaceSize))) * (float(mTabSize) * spaceSize);
                                width = x - cx;
                            }
                            else
                            {
            ...
            unsigned int prevColor = line.empty() ? mPalette[(int)PaletteIndex::Default] : GetGlyphColor(line[0]);
            ImVec2 bufferOffset;

            for (int i = 0; i < (int)line.size();)
            {
                Glyph& glyph = line[i];
                unsigned int color = GetGlyphColor(glyph);
        ...
        if (ImGui::IsMousePosValid() && ImGui::IsWindowHovered())
        {
            ...
            if (!id.empty())
            {
                Identifiers::iterator it = mLanguageDefinition.mIdentifiers.find(id);
                ...
                {
                    Identifiers::iterator pi = mLanguageDefinition.mPreprocIdentifiers.find(id);
...
void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
{
    ...
    if (aChar == '\n')
    {
        ...
        Line& line = mLines[coord.mLine];
        Line& newLine = mLines[coord.mLine + 1];
...
{
    Coordinates oldSelStart = mState.mSelectionStart;
    Coordinates oldSelEnd = mState.mSelectionEnd;
    ...
    {
        const int lineNo = mState.mSelectionEnd.mLine;
...
void TextEditor::InsertText(const char * aValue)
{
    ...
    Coordinates start = std::min(pos, mState.mSelectionStart);
...
void TextEditor::TextEditor::MoveBottom(bool aSelect)
{
    ...
    Coordinates newPos = Coordinates((int)mLines.size() - 1, 0);
...
void TextEditor::Delete()
{
    ...
    else
    {
        ...
        if (pos.mColumn == GetLineMaxColumn(pos.mLine))
        {
            ...
            Line& nextLine = mLines[pos.mLine + 1];
...
void TextEditor::Backspace()
{
    ...
    else
    {
        ...
        if (mState.mCursorPosition.mColumn == 0)
        {
            ...
            Line& line = mLines[mState.mCursorPosition.mLine];
            Line& prevLine = mLines[mState.mCursorPosition.mLine - 1];
...
void TextEditor::Copy()
{
    ...
    else
    {
        ...
        {
            std::string str;
            Line& line = mLines[GetActualCursorCoordinates().mLine];
            for (Glyph& g : line)
    ...
    for (int i = aFromLine; i < endLine; ++i)
    {
        ...
        for (const char * first = bufferBegin; first != last; )
        {
            ...
            if (hasTokenizeResult == false)
            {
                ...
                for (std::pair<std::regex, PaletteIndex>& p : mRegexList)
                {
                    if (std::regex_search(first, last, results, p.first, std::regex_constants::match_continuous))
                    {
                        hasTokenizeResult = true;

                        const std::csub_match& v = *results.begin();
                        token_begin = v.first;
                        token_end = v.second;
...
void TextEditor::ColorizeInternal()
{
    ...
    if (mCheckComments)
    {
        unsigned int endLine = mLines.size();
        int endIndex = 0;
            ...
            if (!line.empty())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        Line::iterator from = line.begin() + currentIndex;
                        std::string& startStr = mLanguageDefinition.mCommentStart;
                        std::string& singleStartStr = mLanguageDefinition.mSingleLineComment;
                        ...
                        std::string& endStr = mLanguageDefinition.mCommentEnd;
...
{
    const Line& line = mLines[aFrom.mLine];
    float distance = 0.0f;
    float spaceSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, " ", nullptr, nullptr).x;
    ...
    for (size_t it = 0u; it < line.size() && it < colIndex; )
    {
        ...
        {
            int d = UTF8CharLength(line[it].mChar);
            char tempCString[7];
            int i = 0;
...
void TextEditor::UndoRecord::Undo(TextEditor * aEditor)
{
    ...
    if (!mRemoved.empty())
    {
        Coordinates start = mRemovedStart;
